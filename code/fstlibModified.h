      ...
      if (next_address) {
        if (prefix.empty() || prefix.front() == arc) {
          depth_first_visit(next_address, word, output, atm, accept,
                            prefix.empty() ? prefix : prefix.substr(1), is_output);
        }
      }
...
public:
  ...
  template <typename T> void enumerate(T callback, bool is_output = false) const {
    matcher<output_t>::depth_first_visit(
        matcher<output_t>::header_.start_address, std::string(), output_t{},
        DummyAutomaton(), callback, std::string_view(), is_output);
  }
};
